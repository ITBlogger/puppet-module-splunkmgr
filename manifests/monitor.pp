...
define splunk::monitor($log, $action) inherits splunk {
  ...
  case $action {
    'add': {
      exec { "add_${name}":
        ...
        command => "/opt/splunkforwarder/bin/splunk add monitor \"${log}\" -auth ${splunk_admin}:${splunk_admin_pass}",
    ...
    'remove': {
      exec { "remove_${name}":
        ...
        command => "/opt/splunkforwarder/bin/splunk remove monitor \"${log}\" -auth ${splunk_admin}:${splunk_admin_pass}",
        notify  => Service['splunk'],
      }
    }
    default: {
